...
qsub -v s3=$s3,dataFile=$dataFile,pathv=$pathv,numberCore=$nthreads,outputpath=$outputpath,s3b=$s3b,parentId=$parentId,annotationFile=$annotationFile,provenanceFile=$provenanceFile -pe mpi $nthreads -S /bin/bash -V -cwd -N  "c3net" -e "$errorOutput/${c3}error.txt" -o "$outOutput/${c3}out.txt" $pathv/networkScripts/c3net.sh
...
qsub -v s3=$s3,dataFile=$dataFile,pathv=$pathv,numberCore=$nthreads,outputpath=$outputpath,s3b=$s3b,parentId=$parentId,annotationFile=$annotationFile,provenanceFile=$provenanceFile -pe mpi $nthreads -S /bin/bash -V -cwd -N  "wgcnaTOM" -e "$errorOutput/${wgcna}error.txt" -o "$outOutput/${wgcna}out.txt" $pathv/networkScripts/wgcnaTOM.sh
