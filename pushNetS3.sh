...
if [ $lassoCV1se -eq "1" ]; then
  ...
  Rscript $pathv/s3LinkToSynapse.R $s3b/lassoCV1seNetwork.csv $outputpath/lassoCV1setempmd5.out $parentId $annotationFile $provenanceFile $bucket "lasso"
...
if [ $ridgeCV1se -eq "1" ]; then
  ...
  Rscript $pathv/s3LinkToSynapse.R $s3b/ridgeCV1seNetwork.csv $outputpath/ridgetempmd5.out $parentId $annotationFile $provenanceFile $bucket "ridge"
